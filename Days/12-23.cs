namespace AdventOfCode
{
  public class Amphipod : IPuzzle
  {
    ...
    internal class c_burrow
    {
      ...
      // Try to send the amphipod in this spot to their home
      ...
      private (bool, int) try_send_hallway_to_home()
      {
        ...
        for (int start_column = 0; start_column < burrow_positions.Length; start_column++)
        {
          ...
          // If we found an amphipod in the hallway, check to see if their path home is clear
          if (start != null)
          {
    ...
    internal void parse_input(
        in string input,
        c_burrow borrow,
        in bool pretty)
    {
      InputReader input_reader = new InputReader();
      var lines = input_reader.GetInputAsString("Amphipod.txt");
      int row = 1;
      for(int i = 2; i < lines.Count; i++){
        var l = lines[i].Substring(1);
        for(int j = 0; j < l.Length; j++){
          char c = l[j];
          if(c >= 'A' && c <= 'D') borrow.place_amphipod(c, row, j);
        }
        row++;
      }
