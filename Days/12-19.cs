...
{
  public class BeaconScanner : IPuzzle
  {
    public void Run(){
      InputReader inputReader = new InputReader();
      var input = inputReader.GetInputAsBlob("BeaconScanner.txt");
      var first = PartOne(input);
      Console.WriteLine("First: " + first);
      var second = PartTwo(input);
    ...
    {
      var scanners = new HashSet<Scanner>(ParseMan(input));
    ...
    private Scanner[] ParseMan(string input){
      List<Scanner> result = new List<Scanner>();
      var block = input.Split("\n\n");
      foreach(var l in block){
        List<Coord> lstCoord = new List<Coord>();
        var line = l.Split("\n").Skip(1);
        foreach(var p in line){
          if(p == string.Empty) continue;
          var parts = p.Split(",");
          var coord = new Coord(int.Parse(parts[0]),int.Parse(parts[1]),int.Parse(parts[2]));
          lstCoord.Add(coord);
        }
        var scan = new Scanner(new Coord(0,0,0), 0, lstCoord);
        result.Add(scan);
      }
